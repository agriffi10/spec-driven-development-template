...
> that one. Reasoning that a fence in `docs/decisions/` rests on lives here or in that fence's entry.
...
`docs/decisions/` entry and nothing more, and a table that grows a row per spec has stopped being an
...
|---|---|---|---|
| 1 | [what changed] | [the one-line situation that forced it] | [pointer to the `docs/decisions/` entry] |
